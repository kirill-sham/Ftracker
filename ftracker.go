...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSp := meanSpeed(action, duration) * kmhInMsec
	spentCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSp, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return spentCalories
}
